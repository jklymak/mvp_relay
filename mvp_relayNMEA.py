        ...
        while serialQueueSize > 0 or udpQueueSize > 0:
            ...
            if getFailed == False:
                ...
                try:
                    ...
                    logging.error('msg_split failed')
                mout = []
                ...
                logging.debug('Relaying')
                # Relay the message if it is of correct format.
                if len(mout)>0:
                    for msg in mout:
                        relayMessage(msg,self)
                else:
                    logging.error('Bad String!')
                    self.Restart=True
...
def clean_nmea_str(nmeaStr):
    ...
    if isGoodStr == True:
        if nmeaStr[0] != '$':
            #        print 'Leading $ is missing' #, so this string is not valid.
            isGoodStr = False
            return nmeaStr, isGoodStr
    #print '$ is there and its long enough'
